...
class QuNeoSelectorComponent(ModeSelectorComponent):
  ...
  def update(self):
    # ModeSelectorComponent.__init__ calls set_mode(0), which calls this,
    # before setup_subselector() has run -- guard against that ordering.
    if getattr(self, '_sub_modes', None) is not None:
